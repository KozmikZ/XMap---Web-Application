...
if user_args.crawl:
    ...
    for link in injectable_urls:
        ...
        for p in link.injection_parameters:
            if user_args.brute:
                for v in ConsoleScanner.scan_url_parameter_brute(str(link),p,depth=sdepth,verbose=user_args.verbose,manual=user_args.manual,payload_list_path=user_args.payload_list):
                    total_xss_vulnerabilities.append(v)
            else:
                for v in ConsoleScanner.scan_url_parameter(str(link),p,depth=sdepth,verbose=user_args.verbose,manual=user_args.manual,payload_list_path=user_args.payload_list):
                    total_xss_vulnerabilities.append(v)
    ...
    if user_args.parameter!=None: # single parameter scanning
        if user_args.brute:
            for v in ConsoleScanner.scan_url_parameter_brute(str(url),user_args.parameter,depth=sdepth,verbose=user_args.verbose,manual=user_args.manual,payload_list_path=user_args.payload_list):
                total_xss_vulnerabilities.append(v)
        else:
        ...
        for p in url.injection_parameters: 
            if user_args.brute:
                ...
                for v in ConsoleScanner.scan_url_parameter(str(url),p,depth=sdepth,verbose=user_args.verbose,manual=user_args.manual,payload_list_path=user_args.payload_list):
                    total_xss_vulnerabilities.append(v)
    if len(total_xss_vulnerabilities)>0:
        print("Found XSS vulnerabilities")
        if user_args.log!=None:
